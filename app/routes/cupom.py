...
async def get_all_cumpoms(db_session: AsyncSession = Depends(db_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cupom not found")




@router.get("/by-id/{cupom_id}", status_code=status.HTTP_200_OK)
async def get_cumpom_by_id( cupom_id: int, db_session: AsyncSession = Depends(db_session)):
    cupom_repository = CupomRepository (db_session, Cupom)
    try:
...
async def get_cumpom_by_name( cupom: str, db_session: AsyncSession = Depends(db_session)):
    cupom_repository = CupomRepository (db_session, Cupom)
    try:
...
async def create_cupom( new_cupom: CupomRequest, db_session: AsyncSession = Depends(db_session)):
    cupom_repository = CupomRepository (db_session, Cupom)
...
async def update_cupom(cupom_id: int, new_cupom: CupomRequest, db_session: AsyncSession = Depends(db_session)):
    cupom_repository = CupomRepository (db_session, Cupom)
    try:
